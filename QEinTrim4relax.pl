...
`rm -rf QE_trimmed4relax`;
`mkdir QE_trimmed4relax`;
...
for my $qe (@QEin_MC){
    ...
    my $trimmed = join("\n",@QE_template);
    chomp $trimmed;
    open(FH, ">QE_trimmed4relax/$QEin_name" ) or die $!;
    print FH $trimmed;
    close(FH);
    `rm -rf QE_trimmed4relax/$prefix`;
    `mkdir QE_trimmed4relax/$prefix`;
    `mv QE_trimmed4relax/$QEin_name QE_trimmed4relax/$prefix/`;
}
#unlink "input.in";
#unlink "output.in";
#
